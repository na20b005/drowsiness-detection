...
while True:
    ...
    for face in faces:
        ...
        # BLINK
        if ear < ear_thresh and closed_prev == False:
                 frame_count = 0
                 closed_prev =True
                # print('eyes closed')
        if ear < ear_thresh and closed_prev == True:
            frame_count += 1
            closed_prev = True
            time_eye_closed = frame_count / 30
            #print('eyes continue to be closed for {} secs '.format(time_eye_closed))
        if closed_prev == True and ear > ear_thresh :
        ...
        # if time_eye_closed >= 1.5 :
        #     print("ALERT MESSAGE eye closed!!")
        ...
        cv2.putText(frame, 'Eye closed for {:.2f} secs'.format(time_eye_closed), (10, 50), font, font_scale, (0, 0, 255), 2)

        # Drowsiness Detection
        if total_yawns > 2 or time_eye_closed > 1.5:
            frame[20:120, w - 120:w - 20] = image  # Show coffee break image
            cv2.putText(frame, "ALERT", (w - 120, 160), font, 1.2, (0, 0, 255), 4)
    ...
    if key == ord('r') or key == ord('R'):
        time_eye_closed, total_yawns = 0, 0  # Reset calculations by pressing 'r' or 'R'
